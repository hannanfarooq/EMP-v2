...
const Q5 = ({ handleNext, handleAnswerChange }) => {
  ...
  const handleNumChildrenChange = (e) => {
    ...
    if (isNaN(value) || value <= 0) {
      ...
      setNumChildren(value);
      setChildrenNames(Array(value).fill(''));
      setChildrenDOBs(Array(value).fill(null)); // Use null for initial state of date picker
    }
    setError(false);
  };
  ...
  const handleSubmit = () => {
    if (hasChild === 'yes') {
      if (numChildren <= 0 || childrenNames.includes('') || childrenDOBs.includes(null)) {
        setError(true);
        return;
      }
    }
          ...
          {Array.from({ length: numChildren }, (_, index) => (
            <Box key={index} sx={{ mb: 2 }}>
              ...
              style={{marginRight:"5px"}}
                ...
                sx={error && childrenNames[index] === '' ? { animation: `${vibration} 0.3s ease` } : {}}
              />
              {/* Custom Date Picker for Child Date of Birth */}
              <DatePicker
                selected={childrenDOBs[index]}
                ...
                customInput={
                  ...
                  style={{marginTop:"17px"}}
                    label={`Child ${index + 1} Date of Birth`}
                    value={childrenDOBs[index] ? childrenDOBs[index].toLocaleDateString("en-GB") : ''}
                    error={error && !childrenDOBs[index]}
                    helperText={error && !childrenDOBs[index] ? "Date of birth is required" : ""}
                    sx={error && !childrenDOBs[index] ? { animation: `${vibration} 0.3s ease` } : {}}
                    InputLabelProps={{
