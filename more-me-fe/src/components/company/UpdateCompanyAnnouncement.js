...
import { createCompanyAnnouncement, UpdateAnnouncement } from "src/api";
...
export default function UpdateCompanyAnnouncement({
  ...
  const handleSubmit = async (event) => {
    ...
    if (Object.keys(formErrors).length === 0) {
      ...
      setSubmitting(true);
      const issubmit = await UpdateAnnouncement(data, storedUserData.token);
      fetchCompanyAnnouncements();
          ...
          >
            <Grid container spacing={2}>
              ...
              <Grid item xs={12}>
                ...
                />
                <span>Select announcement document to update (.pdf/.docx)</span>
